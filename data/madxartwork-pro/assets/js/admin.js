...
jQuery(function () {
	madxartworkProAdmin.assetsManager.fontManager.init();
...
module.exports = function () {
	...
	self.init = function () {
		jQuery(document).on('change', '.madxartwork-widget-template-select', function () {
			...
			var editUrl = madxartworkProConfig.i18n.home_url + '?p=' + templateID + '&madxartwork';
...
var CustomIcons = function (_madxartworkModules$Vie) {
	_inherits(CustomIcons, _madxartworkModules$Vie);
	...
	_createClass(CustomIcons, [{
		...
		value: function bindEvents() {
			...
			madxartworkCommon.elements.$document.on('click', '.remove', triggerDelete);
		...
		value: function showAlertDialog(id, message) {
			...
			return madxartworkCommon.dialogsManager.createWidget('alert', alertData).show();
		}
	}, {
		key: 'onSuccess',
		...
		value: function onInit() {
			...
			var $body = madxartworkCommon.elements.$body,
...
var DropZoneField = function (_madxartworkModules$Vie) {
	_inherits(DropZoneField, _madxartworkModules$Vie);
	...
	_createClass(DropZoneField, [{
		key: 'getDefaultSettings',
		value: function getDefaultSettings() {
			var baseSelector = '.madxartwork-dropzone-field';
			return {
				droppedFiles: false,
				selectors: {
