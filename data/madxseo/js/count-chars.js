 ...
 *          & madxseo General Settings screens.
 *
...
var aiosp_title_extra = parseInt( madxseo_count_chars.aiosp_title_extra );
...
jQuery(document).ready( function() {
	...
	jQuery("#aiosp_description_wrapper").bind("input", function() {
		jQuery("#madxseo_snippet_description").text(jQuery("#aiosp_description_wrapper textarea").val().replace(/<(?:.|\n)*?>/gm, ""));
	});
});

/**
 * madxseo Init Counting
 *
 * @since ?
 */
function madxseoInitCounting(){
	/* count them characters */
	jQuery( '.madxseo_count_chars' ).on('keyup keydown', function(){
		madxseoCountChars( jQuery(this).eq(0), jQuery(this).parent().find('[name="' + jQuery(this).attr('data-length-field') + '"]').eq(0));
	});
	jQuery( '.madxseo_count_chars' ).each(function(){
		madxseoCountChars( jQuery(this).eq(0), jQuery(this).parent().find('[name="' + jQuery(this).attr('data-length-field') + '"]').eq(0));
	});
}
 ...
 * @return Mixed.
 */
function madxseoCountChars( field, cntfield ) {
	var extra = 0;
	var field_size;
	...
	if ( cntfield.val() > field_size ) {
		cntfield.removeClass().addClass('madxseo_count_ugly');
	} else if ( ( 'aiosp_title' === field.attr('name' ) ) || ( 'aiosp_home_title' === field.attr('name') ) ) {
		if ( cntfield.val() > ( field_size - 6 ) ) {
			cntfield.removeClass().addClass('madxseo_count_bad');
		} else {
			cntfield.removeClass().addClass('madxseo_count_good');
		}
	} else {
