/**
 * madx Scroll to element based on its class and highlight it when a menu item is clicked.
 * Copyright (c) madx
 */
;(function( $, window, document ) {
	...
	var pluginName = 'madxFlowScrollHighlight',
		defaults = {
			speed: 900,
			prefix: '.tf_anchor_',
			navigation: '#main-nav',
	...
	Plugin.prototype = {
				...
				if ( $linkHash.length > 0 ) {
					$linkHash.each(function(){
						...
						if ( $link.prop('hash') == hash ) {
							...
							/**
							 * Fires event scrollhighlight.madx
							 * Receives anchor with hash
							 */
							$('body').trigger( 'scrollhighlight.madx', [ hash ] );
							return;
						}
					});
				} 
		...
		linkScroll: function( obj, href ) {
			...
			/**
			 * Fires event scrollhighlightstart.madx before the scroll begins.
			 * Receives anchor with hash.
			 */
			$('body').trigger( 'scrollhighlightstart.madx', [ hash ] );
