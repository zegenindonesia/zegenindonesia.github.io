...
;(function($, window, document, undefined) {
	...
	$('.tf-upgrade-theme').on('click', function(e){
		...
		if ( confirm( tfUpdater.confirm_update ) ) {
			if ( $(this).parent().hasClass( 'login' ) ) {
				...
				$('#madx_update_form').append( '<input type="hidden" name="theme" value="'+ _updater_el.data( 'theme' ) +'" /><input type="hidden" name="package_url" value="'+ _updater_el.data( 'package_url' ) +'" />' ).submit();
	...
	$('.madx_changelogs').on('click', function(e){
		e.preventDefault();
		var $self = $(this),
